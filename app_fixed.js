...
app.post('/', (req, res) => {
  ...
  // limitar longitud para evitar inputs excesivos
  if (!username || !password || username.length > 100 || password.length > 100) {
    return res.status(400).send('Rellena usuario y contraseña (entrada inválida)');
  }

  const conn = db();

  // -------- PATCH: uso de consulta parametrizada para evitar SQLi ----------
  const sql = "SELECT * FROM users WHERE username = ? AND password = ?";
  console.log("Ejecutando SQL parametrizado:", sql, [username, password]);

  conn.get(sql, [username, password], (err, row) => {
    conn.close();
    if (err) {
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`App parcheada escuchando en http://0.0.0.0:${PORT}`);
});
